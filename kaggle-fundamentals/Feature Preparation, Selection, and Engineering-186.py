...
# holdout=pd.concat(dummies,axis=1)
...
feature_importance=pd.Series(coefficients[0],columns)
...
def process_fare(df,cut_points,label_names):
    # df["Fare"] = df["Age"].fillna(-0.5)
    df["Fare_categories"] = pd.cut(df["Fare"],cut_points,labels=label_names)
    return df


cut=[0,12,50,100,1000]
la=['0-12','12-50','50-100','100+']
train=process_fare(train,cut,la)
holdout=process_fare(holdout,cut,la)



cols=["Fare_categories"]
for c in cols:
    train=create_dummies(train,c)
    holdout=create_dummies(holdout,c)
