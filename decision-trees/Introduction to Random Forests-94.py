...
print(roc_auc_score(test['high_income'],numpy.round((sum(predictions))/10)))
...
clf.fit(train[columns], train["high_income"])
trainpredictions = clf.predict(train[columns])
print(roc_auc_score(train["high_income"], trainpredictions))
testpredictions = clf.predict(test[columns])
print(roc_auc_score(test["high_income"], testpredictions))